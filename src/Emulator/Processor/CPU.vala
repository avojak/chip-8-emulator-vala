...
public class CHIP8.Processor.CPU : GLib.Object {

    private const double TIMER_FREQUENCY = 60.0; // Timers are updated a frequency of 60Hz (i.e. once every 16.667ms)
    private double instruction_frequency = CHIP8.Application.settings.instruction_frequency; // instructions/second

    private unowned CHIP8.Memory.MMU mmu;
    private unowned CHIP8.Graphics.PPU ppu;
    private unowned CHIP8.Audio.APU apu;
    private unowned CHIP8.IO.Keypad keypad;

    private CHIP8.Processor.Registers registers;
    ...
    public CPU (CHIP8.Memory.MMU mmu, CHIP8.Graphics.PPU ppu, CHIP8.Audio.APU apu, CHIP8.IO.Keypad keypad) {
        this.mmu = mmu;
        this.ppu = ppu;
        this.apu = apu;
        this.keypad = keypad;
    ...
    construct {
        registers = new CHIP8.Processor.Registers ();
    ...
    private void 0NNN () { } // vala-lint=naming-convention

    // Jump to address NNN
    private void 1NNN (uint16 instruction) { // vala-lint=naming-convention
        registers.pc = get_nnn (instruction);
    }

    // Call subroutine at NNN
    private void 2NNN (uint16 instruction) { // vala-lint=naming-convention
    ...
    private void 3XNN (uint16 instruction) { // vala-lint=naming-convention
    ...
    // Skip next instruction if Vx != NN
    private void 4XNN (uint16 instruction) { // vala-lint=naming-convention
        if (get_vx (instruction) != get_nn (instruction)) {
            next_instruction ();
        }
    }
    ...
    private void 5XY0 (uint16 instruction) { // vala-lint=naming-convention
    ...
    // Set Vx = NN
    private void 6XNN (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, get_nn (instruction));
    }

    // Add NN to Vx (Carry flag is not changed)
    private void 7XNN (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, get_vx (instruction) + get_nn (instruction));
    }

    // Set Vx = Vy
    private void 8XY0 (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, get_vy (instruction));
    }

    // Set Vx = Vx | Vy
    private void 8XY1 (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, get_vx (instruction) | get_vy (instruction));
    }

    // Set Vx = Vx & Vy
    private void 8XY2 (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, get_vx (instruction) & get_vy (instruction));
    }

    // Set Vx = Vx ^ Vy
    private void 8XY3 (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, get_vx (instruction) ^ get_vy (instruction));
    }

    // Set Vx = Vx + Vy
    private void 8XY4 (uint16 instruction) { // vala-lint=naming-convention
    ...
    // Set Vx = Vx - Vy
    private void 8XY5 (uint16 instruction) { // vala-lint=naming-convention
        registers.v[0xF] = get_vx (instruction) > get_vy (instruction) ? 1 : 0;
        set_vx (instruction, get_vx (instruction) - get_vy (instruction));
    }

    private void 8XY6 (uint16 instruction) { // vala-lint=naming-convention
    ...
    // Set Vx = Vy - Vx
    private void 8XY7 (uint16 instruction) { // vala-lint=naming-convention
        registers.v[0xF] = get_vy (instruction) > get_vx (instruction) ? 1 : 0;
        set_vx (instruction, get_vy (instruction) - get_vx (instruction));
    }

    private void 8XYE (uint16 instruction) { // vala-lint=naming-convention
        // TODO: Configurably set Vx = Vy first
    ...
    // Skip next instruction if Vx != Vy
    private void 9XY0 (uint16 instruction) { // vala-lint=naming-convention
        if (get_vx (instruction) != get_vy (instruction)) {
            next_instruction ();
        }
    }
    ...
    // Set I = NNN
    private void ANNN (uint16 instruction) { // vala-lint=naming-convention
        registers.i = get_nnn (instruction);
    }

    // Jump to NNN plus the value in V0
    private void BNNN (uint16 instruction) { // vala-lint=naming-convention
        // TODO: Configurable "quirk" here
        registers.pc = get_nnn (instruction) + registers.v[0];
    }

    // Set Vx = (a random number) & NN
    private void CXNN (uint16 instruction) { // vala-lint=naming-convention
    ...
    // Draw (Vx, Vy, N)
    private void DXYN (uint16 instruction) { // vala-lint=naming-convention
    ...
    private void EX9E (uint16 instruction) { // vala-lint=naming-convention
        if (keypad.is_key_pressed (get_vx (instruction))) {
            next_instruction ();
        }
    }

    private void EXA1 (uint16 instruction) { // vala-lint=naming-convention
        if (!keypad.is_key_pressed (get_vx (instruction))) {
            next_instruction ();
        }
    }
    ...
    // Set Vx = delay timer
    private void FX07 (uint16 instruction) { // vala-lint=naming-convention
        set_vx (instruction, delay_timer);
    }

    private void FX0A (uint16 instruction) { // vala-lint=naming-convention
    ...
    // Set delay timer = Vx
    private void FX15 (uint16 instruction) { // vala-lint=naming-convention
        delay_timer = get_vx (instruction);
    }

    // Set sound timer = Vx
    private void FX18 (uint16 instruction) { // vala-lint=naming-convention
        sound_timer = get_vx (instruction);
    }

    // Add Vx to register I
    private void FX1E (uint16 instruction) { // vala-lint=naming-convention
        // TODO: Check for overflow?
        registers.i += get_vx (instruction);
    }

    private void FX29 (uint16 instruction) { // vala-lint=naming-convention
        registers.i = get_vx (instruction) * CHIP8.Memory.MMU.FONT_SPRITE_SIZE;
    }

    private void FX33 (uint16 instruction) { // vala-lint=naming-convention
    ...
    private void FX55 (uint16 instruction) { // vala-lint=naming-convention
    ...
    private void FX65 (uint16 instruction) { // vala-lint=naming-convention
        for (int i = 0; i <= get_x (instruction); i++) {
